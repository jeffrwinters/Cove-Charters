export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === '/api/v1/boats') {
        return await boats(request, env, cors);
      }

      if (path.startsWith('/api/v1/boats/')) {
        return await boatById(request, env, cors, path.split('/').pop());
      }

      if (path === '/api/v1/settings') {
        return await settings(request, env, cors);
      }

      if (path === '/api/v1/media' || path === '/api/v1/media/upload' || path === '/media/upload' || path === '/upload-media') {
        return await uploadMedia(request, env, cors);
      }
...
async function health(env, cors) {
  ...
  return json({ ok: true, service: 'cove-api', version: '0.3.2', d1: result?.ok === 1 }, 200, cors);
}
...
async function importSeed(env, cors) {
  requireDb(env);
  const seed = await readRepoJson(env, 'data/boats.json');
  let imported = 0;
...
async function uploadMedia(request, env, cors) {
  ...
  if (file.size > Number(env.MAX_UPLOAD_BYTES || 15728640)) {
...
async function readRepoJson(env, path) {
  ...
  if (!response.ok) throw new Error(data.message || 'GitHub read failed');
...
function corsHeaders(request, env) {
  const allowed = (env.ALLOWED_ORIGINS || 'https://jeffrwinters.github.io,https://covecharters.com,https://www.covecharters.com')
    .split(',')
    .map(item => item.trim());
...
function cleanFilename(value) {
  ...
  return `${cleanSegment(parts.join('.') || 'upload')}.${ext}`;
}
